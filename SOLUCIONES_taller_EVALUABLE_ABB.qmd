...
```{r}
plot(tbl_d2$Log_Rank, tbl_d2$Log_Freq, main="Zipf - Descriptions(Mallorca)", xlab="loq(Rank)", ylab="log(Freq)", pch=19, col="darkgray")

abline(sol_d, col="blue", lwd=2)
```

Para analizar si la ley de Zipf se ajusta a los daros, se ha estudiado la relacion entre la frecuencia de las longitudes de los comentarios i su rango. Segun la ley de Zipf, esta realcion tiene que ser de tipo potencial, cosa que implica una relacion lineal entre loq(Freq) i log(Rank).

En ambos casos (reviews i descriptions), la regresión lineal del model $log(Freq)$ \~ $loq(Rank)$ muestra un coeficiente negativo i estadísticamente significativo, con un valor de $R^2$ moderadamente elevado.
