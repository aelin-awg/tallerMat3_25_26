...
En cuanto a funciones usadas para la creacion del grafico:
-   aes: srive para definir que variable corresponde con el eje de coordenadas X e Y al igual que determinar el color de esta. Es una funcion estetica.

-   geom_boxplot: Seleccionamos la forma de representacion de nuestras distribuciones.

-   labs, theme_minimal, theme: Sirven para añadir cosas como etiquetas y titulos, colores...
En general, cada una de estas funciones sirven para añadir funciones o elementos visuales o esteticos.
Al usar boxplot como nos pide el enunciado, se nos hace mas facil la comparacion por cosas como la muestra mediana, permitiendo una comparacion mas directa, la comparacion de multiples distribuciones a la vez y la señalizacion de los outliers.
**Interpretación del gráfico y Comparación entre municipios.**
Dentro de nuestro grafico, cada Municipio vendra representado por un boxplot de un color distinto cada uno. Dentro de este grafico, la lina horizontal dentro de la caja es la mediana, el valor central. La caja abarca de nuestro primer cuartil hasta el tercero, que representa el 50% central de los datos. Los bigotes, que son las barras verticales de la figura, sirven para indicar los valores que no se consideran outliers, mientras que los puntos negros fuera de estos son los outliers, con valoraciones inusualmente bajas o altas.
Alcúdia presenta una mediana alta, que ronda el 4.8. Como su caja es estrecha, esto nos indica que hay una variabilidad baja. Hay una gran cantidad de outliers, la mayoria de los cuales suelen rondar puntuacions altas, pero hay un subconjunto de ellos que ronda las bajas.
En cuanto a Calviá, su mediana ronda los 4.8 o 4.85, pero se diferencia de Alcúdia con una caja mas ancha, indicando una variabilidad mas alta. En cuanto a los Outliers, Calviá presenta pocos, y ademas rondan puntuaciones altas iguales o superiores al 3.
Despues se tiene a Palma de Mallorca, la cual presenta una mediana similar a la de Calviá, pero con una caja mas estrecha indicando poca variabilidad, es decir, la mayoria de valoraciones suelen rondar una media de 4.8 o 4.85. Aunque tenga mas outliers, estos estan en una situacion casi identica a los de Calviá, donde rondan puntuaciones altas iguales o superiores al 3.
Por ultimo, Pollença presenta una mediana similar a la de Alcúdia, pero con una caja mas ancha indicando mas variabilidad. No solo eso, sino que sus outliers estan dispersos por todo el eje Y, con valoraciones que llegan hasta el 1.
En conclusion, Palma de Mallorca es la mejor valorada de las cuatro, pero por muy poco ya que Calviá esta en una situación similar. En tercer lugar estaria Alcúdia y por ultimo Pollença.
...
$H_0 : p_1 = p_2$
$H_1 : p_1 \ne p_2$
Establecienco nuestro nivel de significacion $\alpha = 0.05$. Si el p-valor $\geq$ 0.05, no rechazamos $H_0$, que se puede ver tambien a la hora de hacer el intervalo de confianza por si apareze el 0 o no.
Comenzamos filtrando nuestros datos por fecha y municipios:
```{r}
...
```
x = Numero de apartamentos con rating > 4
n = numero total de apartamentos
$\hat p$ = propocion $x/n$
Ahora, guardamos cada valor calculado en su respectivo lugar haciendo uso de filtros.
```{r}
# Alcúdia
...
El intervalo de confianza para el caso XIX es
$\displaystyle ]\hat p_1 - \hat p_2-z_{0.975}·\sqrt{\hat p \hat q (\frac{1}{n_1}+\frac{1}{n_2})},\hat p_1 \hat p_2 + z_{0.975}\sqrt{\hat p \hat q (\frac{1}{n_1}+\frac{1}{n_2})}[$.
Este lo podemos calcular de la siguiente forma:

```{r}
c(p1-p2-qnorm(0.975)*sqrt(p*q*(1/n1+1/n2)),p1-p2+qnorm(0.975)*sqrt(p*q*(1/n1+1/n2))) #0.975 sale de hacer 1-(alpha=0.05/2)
```
Nuestro intervalo de confianza para nuestra diferencia es [-0.02712, 0.04813]. Como podemos ver, el 0 acaba estando incluido en este intervalo, con lo que podemos corroborar que las dos proporciones son iguales, ya que no hay pruebas de una diferencia significativa.
Por lo que, en conclusion, no encontramos pruebas estadisticamente significativas de que la proporcion de apartamentos con `review_scores_rating > 4` difiera entre Alcúdia y Calvià en la muestra de la fecha 21-09-2025.
...
$H_0 : p_1 = p_2$
$H_1 : p_1 \ne p_2$
Establecienco nuestro nivel de significacion $\alpha = 0.05$. Si el p-valor $\geq$ 0.05, no rechazamos $H_0$, que se puede ver tambien a la hora de hacer el intervalo de confianza por si apareze el 0 o no dentro del intervalo.
Comenzamos filtrando nuestros datos por fecha y municipios:
```{r}
...
2*pnorm(Z, lower.tail = FALSE)
```
Nuestro p-valor (1.77824e^-06), nos sale inferior al $\alpha = 0.05$ establecido anteriormente, por lo que rechazamos nuestra hipotesis nula, aceptando la alternativa, y por lo tanto podemos decir que las proporciones entre Palma de Mallorca y Pollença son diferentes para el periodo de tiempo de 2025-06-15.
Para la comparacion final entre ambos, calcularemos el intervalo de confianza del 95%  usando la formula del caso XIX: $\displaystyle ]\hat p_1 - \hat p_2-z_{0.975}·\sqrt{\hat p \hat q (\frac{1}{n_1}+\frac{1}{n_2})},\hat p_1 \hat p_2 + z_{0.975}\sqrt{\hat p \hat q (\frac{1}{n_1}+\frac{1}{n_2})}[$.
```{r}
...
2*pnorm(Z, lower.tail = FALSE)
```
Nuestro p-valor (1.762434e^-06), nos sale inferior al $\alpha = 0.05$ establecido anteriormente, por lo que rechazamos nuestra hipotesis nula, aceptando la alternativa, y por lo tanto podemos decir que las proporciones entre Palma de Mallorca y Pollença son diferentes para el periodo de tiempo de 2025-09-21.
Para la comparacion final entre ambos, calcularemos el intervalo de confianza del 95%  usando la formula del caso XIX: $\displaystyle ]\hat p_1 - \hat p_2-z_{0.975}·\sqrt{\hat p \hat q (\frac{1}{n_1}+\frac{1}{n_2})},\hat p_1 \hat p_2 + z_{0.975}\sqrt{\hat p \hat q (\frac{1}{n_1}+\frac{1}{n_2})}[$.
```{r}
...
interval2
```
El intervalo del periodo 2025-06-15 es: [0.03738, 0.08939], mientras que el del periodo 2025-09-21 es: [0.03584, 0.08568]. De forma apartada, ambos intervalos no incluyen el 0, por lo que corroboramos lo mencionado anteriormente de que rechazamos la hipotesis nula y adoptamos la alternativa en ambos casos.
