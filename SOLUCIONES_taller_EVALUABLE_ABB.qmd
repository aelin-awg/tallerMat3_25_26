...
tbl_d = tibble(L=as.numeric(names(aux_d)), Freq=as.numeric(aux_d), Rank=rank(-Freq, ties.method="first"), Log_Freq=log(Freq), Log_Rank=log(Rank))


tbl_d2=tbl_d %>% filter(Rank > 10) %>% filter(Rank < 1000)
...
Para analizar si la ley de Zipf se ajusta a los datos, se ha estudiado la relacion entre la frecuencia de las longitudes de los comentarios y su rango. Segun la ley de Zipf, esta realcion tiene que ser de tipo potencial, cosa que implica una relacion lineal entre log(Freq) y log(Rank).

En ambos casos (reviews y descriptions), la regresión lineal del model $log(Freq)$ \~ $log(Rank)$ muestra un coeficiente negativo y estadísticamente significativo, con un valor de $R^2$ moderadamente elevado.

Gráficamente, los puntos en escala log-log se alinean aproximadamente en torno a una recta, indicando que los datos siguen un comportamiento compatible con la ley de Zipf, aunque con desviaciones en los extremos del rango, hecho habitual en datos empíricos reales.

En conclusión, los resultados obtenidos muestran que la distribución de longitudes tanto en los comentarios como en las descripciones de los apartamentos de Mallorca siguen un patrón compatible con la lley de Zipf, con una relación log-log significativa y una alineación gráfica clara.
